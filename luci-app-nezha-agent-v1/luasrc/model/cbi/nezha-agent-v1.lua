...
function m.on_commit(self)
    ...
    local config_mode = uci:get("nezha-agent-v1", "config", "config_mode") or "preset"
    local enabled = uci:get("nezha-agent-v1", "config", "enabled") == "1"
    
    local result = true
    
    if config_mode == "preset" then
        -- 只在preset模式下更新/etc/config/nz.yml
        local data = {
            -- 基本设置
        ...
        -- 写入新配置
        result = write_yaml(data)
    else
        -- custom模式，确保自定义配置文件存在
        local custom_config_file = "/etc/config/nz_custom.yml"
        if not fs.access(custom_config_file) then
            -- 如果自定义配置文件不存在，创建一个默认的
            local default_config = io.open("/etc/config/nz.yml", "r")
            if default_config then
                local content = default_config:read("*all")
                default_config:close()
                fs.writefile(custom_config_file, content)
                nixio.syslog("debug", "Created default custom config file")
            else
                nixio.syslog("err", "Default config file not found, cannot create custom config")
                result = false
            end
        end
    end
    ...
    if enabled then
        if result then
            ...
            nixio.syslog("err", "Failed to write configuration, but still attempting to start nezha-agent")
            luci.sys.call("/etc/init.d/nezha-agent restart >/dev/null 2>&1")
        end
    else
        nixio.syslog("debug", "Configuration saved, but nezha-agent is disabled, stopping service")
...
function o.validate(self, value, section)
    local config_mode = uci:get("nezha-agent-v1", "config", "config_mode") or "preset"
    if config_mode == "preset" and (value == nil or value == "") then
        return nil, translate("面板地址不能为空")
    end
    return value
end
...
function o.validate(self, value, section)
    local config_mode = uci:get("nezha-agent-v1", "config", "config_mode") or "preset"
    if config_mode == "preset" and (value == nil or value == "") then
        return nil, translate("客户端密钥不能为空")
    end
    return value
end
o:depends("config_mode", "preset")
